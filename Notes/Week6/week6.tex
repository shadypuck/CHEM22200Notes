...
\begin{itemize}
    \item \marginnote{5/5:}Today's lecture content in \textcite{bib:SolomonsEtAl}.
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Liberating \ce{CO2} from a $\beta$-diketoester: \ce{NaOH} then \ce{H3O+} \emph{or} \ce{H3O+} and heat.
    \end{itemize}
    ...
    \begin{center}
        ...
        \setchemfig{atom sep=1.4em}
        \schemestart
            \chemfig{-[:-30](-[6])-[:30](=[2]O)-[:-30]OEt}
            \arrow{->[1. LDA, \SI{-78}{\celsius}][2. \ce{PhCOCl}\rule{6.2mm}{0pt}]}[,2]
            \chemfig{(-[:-150]*6(-=-=-=))(=[2]O)-[:-30](-[:-70])(-[:-110])-[:30](=[2]O)(-[:-30]OEt)}
        \schemestop
    \end{center}
    \begin{itemize}
    ...
    \begin{center}
        ...
        \setchemfig{atom sep=1.4em}
        \schemestart
            \chemfig{-[:-30](-[6]H)-[:30](=[2]O)-[:-30]-[:30]}
            \+
            \chemfig{-[:-30](-[6]H)-[:30](=[2]O)-[:-30]OEt}
            \arrow{->[1. \ce{NaOEt}][2. \ce{H3O+}\rule{1mm}{0pt}]}[,1.4]
            \chemfig{-[:-30]-[:30](=[2]O)-[:-30](-[6])-[:30](=[2]O)-[:-30]-[:30]}
        \schemestop
    \end{center}
    \item Mechanism.
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize}
    \item Tang postulates that acid and heat is the way to liberate \ce{CO2} from $\beta$-ketoesters?
    ...
    \begin{center}
        ...
        \setchemfig{atom sep=1.4em}
        \schemestart
            \chemfig{EtO-[:30]@{C1a}(=[2]O)-[:-30]@{C1b}-[:30]@{C1c}-[:-30]@{C1d}-[:30]@{C1e}-[:-30]@{C1f}-[:30]@{C1g}(=[2]O)-[:-30]OEt}
            \arrow{->[1. \ce{NaOEt}][2. \ce{H3O+}\rule{1mm}{0pt}]}[,1.4]
            \chemfig{*6(@{C2a}-@{C2b}-@{C2c}-@{C2d}(-@{C2e}(=[2]O)-[:-30]OEt)-@{C2f}(=O)-@{C2g}-)}
        \schemestop
        \chemmove{
            \foreach \nod/\n in {C1a/1,C1b/2,C1c/3,C1d/4,C1e/5,C1f/6,C1g/7} {
                \node [below,font=\scriptsize\color{orx}] at (\nod) {\n};
            }
            \node [below,font=\scriptsize\color{orx}] at (C2a) {3};
            \node [below,font=\scriptsize\color{orx}] at (C2b) {4};
            \node [below,font=\scriptsize\color{orx}] at (C2c) {5};
            \node [above,font=\scriptsize\color{orx}] at (C2d) {6};
            \node [below,font=\scriptsize\color{orx}] at (C2e) {7};
            \node [below,font=\scriptsize\color{orx}] at (C2f) {1};
            \node [above,font=\scriptsize\color{orx}] at (C2g) {2};
        }
    \end{center}
    \item Mechanism.
    \begin{itemize}
        \item The mechanism is entirely analogous to Figure \ref{fig:mechanismClaisen}. As drawn, we deprotonate C6, and then the enolate on the right attacks the ester on the left.
    \end{itemize}
    \item Asymmetric diesters.
    \begin{figure}[h!]
        \centering
        \footnotesize
        \schemestart
            \chemfig{EtO-[:30]@{C1a}(=[2]O)-[:-30]@{C1b}-[:30]@{C1c}-[:-30]@{C1d}-[:30]@{C1e}-[:-30]@{C1f}(-[6])-[:30]@{C1g}(=[2]O)-[:-30]OEt}
            \arrow(1--2){->[1. \ce{NaOEt}][2. \ce{H3O+}\rule{1mm}{0pt}]}[,1.4]
            \chemfig{*6(@{C2a}-@{C2b}-@{C2c}-@{C2d}(-@{C2e}(=[2]O)-[:-30]OEt)(-[2]H)-@{C2f}(=O)-@{C2g}(-)-)}
            \arrow(@1--3){-/>[*{0.-90}{1. \ce{NaOEt}\rule[-1mm]{0pt}{1cm}}][*{0.90}2. \ce{H3O+}\rule{1mm}{0pt}\rule{0pt}{3mm}]}[180,1.4]
            \chemfig{*6(@{C3a}-@{C3b}-@{C3c}-@{C3d}(-@{C3e}(=[2]O)-[:-30]OEt)(-[2])-@{C3f}(=O)-@{C3g}-)}
        \schemestop
        \chemmove{
            \node [below,font=\scriptsize\color{orx}] at (C1a) {1};
            \node [above,font=\scriptsize\color{orx}] at (C1b) {2};
            \node [below,font=\scriptsize\color{orx}] at (C1c) {3};
            \node [above,font=\scriptsize\color{orx}] at (C1d) {4};
            \node [below,font=\scriptsize\color{orx}] at (C1e) {5};
            \node [above,font=\scriptsize\color{orx}] at (C1f) {6};
            \node [below,font=\scriptsize\color{orx}] at (C1g) {7};
            %
            \node [anchor=-150,font=\scriptsize\color{orx}] at (C2a) {5};
            \node [anchor=-90 ,font=\scriptsize\color{orx}] at (C2b) {4};
            \node [anchor=-30 ,font=\scriptsize\color{orx}] at (C2c) {3};
            \node [anchor=30  ,font=\scriptsize\color{orx}] at (C2d) {2};
    ...
    \item Retro-Claisen condensations.
    \begin{itemize}
        \item Suppose we subject a $\beta$-diketoester to Claisen condensation conditions (e.g., excess \ce{EtO-}).
        \item If there is a middle $\alpha$-hydrogen, we will preferentially deprotonate and form a stable enolate until workup.
        \item If there is \emph{not} a middle $\alpha$-hydrogen, \ce{EtO-} can add into the ketone, forming intermediate 2 of Figure \ref{fig:mechanismClaisen}. From here, we can react backwards to intermediates 2 and then the starting material.
        \begin{itemize}
        ...
        \begin{itemize}
            \item Stated another way, if you put either ethyl acetate or its Claisen condensation product under \ce{NaOEt}, we will get the Claisen condensation product. However, if we put either ethyl isobutyrate or its Claisen condensation product under \ce{NaOEt}, we will get the ester.
            \item Tang will give more examples next lecture.
        \end{itemize}
    \end{itemize}
    \item Conjugate addition.
    \item Tang draws three resonance structures for an $\alpha,\beta$ unsaturated compound: One neutral, the next one with the \ce{C=O} bond shifted up onto the oxygen (positive carbonyl carbon), and the next one with the double bond flipped (positive $\beta$ carbon).
    \begin{itemize}
    ...
    \begin{center}
        ...
        \setchemfig{atom sep=1.4em}
        \schemestart
            \chemfig{=^[:-30]-[:30](=[2]O)-[:-30]R}
            \arrow{->[\ce{Nu}]}
            \chemfig{Nu-[:30]-[:-30]-[:30](=[2]O)-[:-30]R}
        \schemestop
    \end{center}
    \begin{itemize}
        \item The overall picture looks like we added \ce{HNu} to the double bond, but mechanistically, this reaction will nor proceed without the presence of the carbonyl.
    \end{itemize}
    \item Mechanism.
    \begin{figure}[h!]
        \centering
        ...
        \footnotesize
        \schemestart
            \chemfig{@{C1}=^[@{db1a}:-30]-[@{sb1}:30](=[@{db1b}2]@{O1}O)-[:-30]R}
            \arrow{->[\chemfig{@{Nu2}\charge{90=\:,45:3pt=$\ominus$}{Nu}}]}
            \chemfig{Nu-[:30]-[:-30]=^[@{db3}:30](-[@{sb3}2]@{O3}\charge{45:1pt=$\ominus$}{O})-[:-30]R}
            \arrow{->[\chemfig[atom sep=1.4em]{@{H4}H-[@{sb4}]@{Nu4}Nu}]}[,1.2]
            \chemfig{Nu-[:30]-[:-30]-[:30](=[2]O)-[:-30]R}
        \schemestop
        \chemmove{
            \draw [curved arrow={6pt}{4pt}] (Nu2) to[out=90,in=70,looseness=1.5] (C1);
            \draw [curved arrow={4pt}{2pt}] (db1a) to[out=60,in=120,looseness=2] (sb1);
            \draw [curved arrow={3pt}{2pt}] (db1b) to[bend right=90,looseness=3] (O1);
            \draw [curved arrow={11pt}{2pt}] (O3) to[out=45,in=0,looseness=5] (sb3);
            \draw [curved arrow={4pt}{2pt}] (db3) to[out=120,in=90,looseness=3] (H4);
            \draw [curved arrow={3pt}{2pt}] (sb4) to[bend left=90,looseness=3] (Nu4);
        }
        \caption{Conjugate addition mechanism.}
        \label{fig:mechanismConjAdd}
    \end{figure}
    \item Review.
    \begin{itemize}
        \item Reagents that do 1,2-addition: Lithiates, Grignards (mostly), and \ce{LiAlH4}.
        \item Reagents that do 1,4-addition: \ce{H2O}, \ce{R-SH}, \ce{RNH2}, \ce{CN-}, and \ce{R2CuLi}.
    \end{itemize}
    \item \textbf{Michael addition}: Conjugate addition with an enolate serving as the nucleophile. \emph{Also known as} \textbf{1,4-addition}, \textbf{Michael-type addition}.
    \item General form.
    \begin{center}
        \footnotesize
        \setchemfig{atom sep=1.4em}
        \schemestart
            ...
            \chemfig{=^[:-30]-[:30](=[2]O)-[:-30]R}
            \arrow{->[\ce{NaOEt}]}[,1.1]
            \chemfig{-[:30](=[2]O)-[:-30](-[6]-[:-30]-[6](=[:-30]O)(-[:-150]EtO))-[:30](=[2]O)-[:-30]OEt}
        \schemestop
    \end{center}
    \begin{itemize}
    ...
    \begin{center}
        ...
        \schemestart
            \chemfig{*6(----(=O)--)}
            \arrow{0}[,0.1]\+
            \chemfig{=^[:-30]-[:30](=[2]O)-[:-30]}
            \arrow{->[\ce{NaOEt}][\ce{EtOH}]}[,1.1]
            \chemfig{*6(--(*6(---(=O)-=))----)}
        \schemestop
    \end{center}
    \begin{itemize}
        \item A one-pot reaction of Michael and aldol.
        \item Methyl vinyl ketone (MVK) is a key reactant in Robinson annulations.
    \end{itemize}
    \item Mechanism.
    \begin{figure}[H]
        \centering
        ...
        \footnotesize
        \schemestart
            \chemfig{*6(---(-[@{sb1a}]@{H1}H)-[@{sb1b}](=[@{db1}]@{O1}O)--)}
            \arrow{<=>[\chemfig{@{O2}\charge{90:3pt=$\ominus$}{O}Et}][\ce{EtOH}]}
            \chemfig{*6(---=[@{db3}](-[@{sb3}]@{O3}\charge{180=\:,45:1pt=$\ominus$}{O})--)}
            \arrow{->[\chemfig[atom sep=1.4em]{@{C4}=^[@{db4a}:-30]-[@{sb4}:30](=[@{db4b}2]@{O4}O)-[:-30]}]}[,1.3]
            \chemfig{*6(---(--[:-30]=^[@{db5}](-[@{sb5}2]@{O5}\charge{45:1pt=$\ominus$}{O})-[:-30])-(=O)--)}
            \arrow{->[\chemfig[atom sep=1.4em]{@{H6}H-[@{sb6}]@{O6}OEt}][-\ce{EtO-}]}[,1.3]
            \chemfig{*6(---(--[:-30]-(=[@{db7}2]@{O7}O)-[@{sb7a}:-30]-[@{sb7b}]@{H7}H)-(=O)--)}
            \arrow{<=>[*{0}\chemfig{Et@{O8}\charge{45:1pt=$\ominus$}{O}}][*{0}\ce{EtOH}]}[-90]
            \chemfig{*6(--(--[:30]-(-[@{sb9}6]@{O9a}\charge{180=\:,45:1pt=$\ominus$}{O})=^[@{db9a}:30])-@{C9}(=[@{db9b}]@{O9b}O)---)}
            \arrow{<=>}[180]
            \chemfig{*6(--(*6(---(=O)--))-(-[2]\charge{45:1pt=$\ominus$}{O})---)}
            \arrow[180]
            \chemfig{*6(--(*6(---(=O)-=))----)}
        \schemestop
        \chemmove{
            \draw [curved arrow={11pt}{2pt}] (O2) to[out=90,in=90,looseness=3] (H1);
            \draw [curved arrow={2pt}{2pt}] (sb1a) to[bend right=60,looseness=1.8] (sb1b);
            \draw [curved arrow={3pt}{2pt}] (db1) to[bend left=90,looseness=3] (O1);
            \draw [curved arrow={6pt}{2pt}] (O3) to[bend right=90,looseness=3] (sb3);
            \draw [curved arrow={2pt}{2pt}] (db3) to[out=60,in=120,looseness=1.5] (C4);
            \draw [curved arrow={4pt}{2pt}] (db4a) to[bend left=60,looseness=1.8] (sb4);
            \draw [curved arrow={3pt}{2pt}] (db4b) to[bend right=90,looseness=3] (O4);
    ...
    \begin{center}
        ...
        \schemestart
            ...
            \arrow{->[\ce{HCl}]}
            \chemfig{-[:30](=[2]O)-[:-30]-[:30]-[:-30]N(-[6])-[:30]}
        \schemestop
    \end{center}
    \begin{itemize}
        \item The key components here are (1) a ketone, (2) an aldehyde (typically formaldehyde), (3) a \emph{secondary} amine, (4) acidic conditions, and (5) the ability to drive off \ce{H2O}.
    \end{itemize}
    \item Mechanism.
    \begin{itemize}
    ...
    \begin{center}
        ...
        \schemestart
            \chemfig{-[:30](=[2]O)-[:-30]-[:30]-[:-30]N(-[6])-[:30]}
            \arrow{->[1. \ce{MeI}\rule{6.6mm}{0pt}][2. \ce{Ag2O}, $\Delta$]}[,1.5]
            \chemfig{-[:30](=[2]O)-[:-30]=^[:30]}
        \schemestop
    \end{center}
    \begin{itemize}
        \item Nitrogen attacks \ce{MeI} to form an \ce{NMe3R+} species. It is this molecule that reacts with silver oxide.
    \end{itemize}
